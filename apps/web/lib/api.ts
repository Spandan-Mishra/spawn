...
const getFiles = async ({
  ...
  const response = await axios.get(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/project/${projectId}/files`,
  );
...
const startSandbox = async ({
  ...
  const response = await axios.post(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/project/${projectId}/startSandbox`,
  );
  return response.data;
};
...
const streamChat = async (projectId: string, payload: string) => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/project/${projectId}/chat`,
    {
      method: "POST",
      headers: {
